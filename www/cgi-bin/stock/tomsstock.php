...
class TomsStock extends MYDB2
{
     ...
     * トムス商品の在庫情報を更新
     *
     * @param array args {'item_code','color_code','size_name','amount','jancode'}
     * @return string|null
     */
     ...
     * @return void
     */
    public function update_toms_master($args)
    {
        try {
            if (empty($args)) {
                return;
            ...
            // 既存の在庫レコードの有無を確認
            $sql1 = "select count(*) as cnt from tomsmaster where jan_code=?";
            $stmt_count = $conn->prepare($sql1);
            ...
            // 新規登録
            $sql4 = "insert into tomsmaster(tomsid, toms_item_code, toms_item_name, toms_color_code, toms_color_name, toms_size_code, toms_size_name, jan_code, updated_at) values(null,?,?,?,?,?,?,?,?)";
            $stmt_insert = $conn->prepare($sql4);

            $len = count($args);
            for ($i = 0; $i < $len; $i++) {
                $stmt_count->bind_param("s", $args[$i]['jan_code']);
                $stmt_count->execute();
                $stmt_count->store_result();
                $rec = parent::fetchAll($stmt_count);
            ...
            // トムスマスターの未更新アイテムを削除
            $sql = "delete from tomsmaster where updated_at < '".$update_time."'";
            $result = $conn->query($sql);
        } finally {
            $stmt_count->close();
            $stmt_update->close();
...
if (empty($data)) {
    ...
    // tomsmaster更新
    if (!empty($tomsMaster)) {
        $inst->update_toms_master($tomsMaster);
    }
