...
class Tag implements Master {
	...
	public function insert(...$args): bool
	{
		try {
			...
			$sql = "insert into tags (tag_name, tag_type, tag_order) values (?, ?, ?)";
			for ($i=0, $len=count($args); $i<$len; $i++) {
				$this->_sql->prepared($sql, "sii", array($args[$i][0], $args[$i][1], $args[$i][2]));
			}
		} catch (Exception $e) {
			$res = false;
		}
	...
	public function delete($key): bool
	{
		try {
			...
			$sql = "delete from tags where tagid=? limit 1";
