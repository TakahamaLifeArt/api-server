...
class ItemGroup2 implements Master {
	...
	public function update(...$args): bool
	{
		try {
			...
			$sql = "update item_group2 set name=? where id=?";
	...
	public function insert(...$args): bool
	{
		try {
			...
			if (empty($args)) throw new Exception();
			$now = date('Y-m-d');
			$sql = "insert into item_group2 (name, group2_apply) values (?, ?)";
	...
	public function delete($key): bool
	{
		try {
			...
			$sql = "delete from item_group2 where id=? limit 1";
