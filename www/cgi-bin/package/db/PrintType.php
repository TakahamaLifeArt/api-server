...
class PrintType implements Master {
	...
	public function insert(...$args): bool
	{
		try {
			...
			$sql = "insert into printtype (printtype_name, printtype_key) values (?, ?)";
	...
	public function delete($key): bool
	{
		try {
			...
			$sql = "delete from printtype where printtype_key=? limit 1";
			$this->_sql->prepared($sql, "s", array($key));
		} catch (Exception $e) {
			$res = false;
		}
		return $res;
