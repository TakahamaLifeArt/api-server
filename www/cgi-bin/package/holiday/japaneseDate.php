...
class japaneseDate
{
	...
	*/
	...
	function getVrenalEquinoxDay($year)
	{
		...
		return mktime(0, 0, 0, JD_VERNAL_EQUINOX_DAY_MONTH, (int)$day, (int)$year);
	}
	...
	function getAutumnEquinoxDay($year)
	{
		...
		return mktime(0, 0, 0, JD_AUTUMNAL_EQUINOX_DAY_MONTH, (int)$day, (int)$year);
	}
	...
	{
		return (int)date("w", (int)$time_stamp);
	}
	...
	{
		return (int)date("Y", (int)$time_stamp);
	}
	...
	{
		return (int)date("n", (int)$time_stamp);
	}
	...
	{
		return (int)date("j", (int)$time_stamp);
	}
	...
	{
		return (int)date("d", (int)$time_stamp);
	}
	...
	{
		return ((int)date("j", (int)$time_stamp)+(int)date("m", (int)$time_stamp)) % 6;
	}
	...
	function getWorkingDay($time_stamp, $lim_day, $luna = true, $is_bypass_holiday = true, $bypass_week_arr = false, $is_bypass_date = false )
	{
		...
		if (is_array($is_bypass_date)) {
			...
			foreach ($is_bypass_date as $value) {
				...
				$gc[mktime(0, 0, 0, (int)date("m", $value), (int)date("d", $value), (int)date("Y", $value))] = 1;
			}
			$is_bypass_date = $gc;
		} else {
		...
		$res = array();
		$i = 0;
		$year  = date("Y", (int)$time_stamp);
		$month = date("m", (int)$time_stamp);
		$day   = date("d", (int)$time_stamp);
		while (count($res) != $lim_day) {
			$time_stamp = mktime(0, 0, 0, (int)$month, (int)$day + $i, (int)$year);
	...
	function getSpanCalendar($year, $month, $str, $lim, $luna = true)
	{
		...
		if ($luna == false) {
			while ($lim != 0) {
				$time_stamp = mktime(0, 0, 0, (int)date("m", $time_stamp), (int)date("d", $time_stamp) + 1, (int)date("Y", $time_stamp));
	...
	function purseTime($time_stamp, $luna = true)
	{
		...
		$day = date("j", (int)$time_stamp);
		$res = array(
			"time_stamp" => $time_stamp, 
			"day"        => $day, 
			"strday"     => date("d", (int)$time_stamp), 
